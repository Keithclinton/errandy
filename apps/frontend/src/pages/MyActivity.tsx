    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild size="sm">
          <Link to="/listings/new">Post an errand</Link>
        </Button>
      </div>
      ...
      <Tabs defaultValue="listings">
        <TabsList>
          ...
          <TabsTrigger value="placed">Bids placed</TabsTrigger>
        ...
        <TabsContent value="listings" className="space-y-4">
          ...
          {myListings.data && myListings.data.items.length === 0 && (
            <p className="py-8 text-center text-sm text-muted-foreground">You haven't posted any errands yet.</p>
          )}
          {myListings.data && myListings.data.items.length > 0 && (
        ...
        <TabsContent value="placed" className="space-y-4">
          ...
          {myBids.data && myBids.data.length === 0 && (
            <p className="py-8 text-center text-sm text-muted-foreground">You haven't placed any bids yet.</p>
          )}
          {myBids.data && myBids.data.length > 0 && (
        ...
        <TabsContent value="received" className="space-y-4">
          ...
          {receivedBids.data && receivedBids.data.length === 0 && (
            <p className="py-8 text-center text-sm text-muted-foreground">No bids on your listings yet.</p>
          )}
          {receivedBids.data && receivedBids.data.length > 0 && (
